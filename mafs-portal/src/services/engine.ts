...
// From dashboard engine — required by ClaimsTool.tsx
// ─────────────────────────────────────────────
export const calculateClaimPayout = (
  ...
  // 1. Check Per Incident Limit
  let coveredAmount = Math.min(claimAmount, limits.perIncident);
  if (claimAmount > limits.perIncident) {
    notes.push(`Claim exceeded per-incident limit of $${limits.perIncident.toLocaleString()}.`);
  }

  // 2. Apply Deductible
  const deductible = limits.deductible;
  let afterDeductible = Math.max(0, coveredAmount - deductible);

  // 3. Apply Coinsurance
  let fundPays = afterDeductible * (1 - limits.coinsurance);
  let memberPays = (afterDeductible * limits.coinsurance) + deductible;

  // Add back the amount exceeding per-incident limit to member responsibility
  if (claimAmount > limits.perIncident) {
    memberPays += (claimAmount - limits.perIncident);
  }

  // 4. Check Annual Max
  if (annualClaimsYtd + fundPays > limits.annualMax) {
    const remainingAnnualLimit = Math.max(0, limits.annualMax - annualClaimsYtd);
    const originalFundPays = fundPays;
    fundPays = Math.min(fundPays, remainingAnnualLimit);
    memberPays += (originalFundPays - fundPays);
    notes.push(`Annual maximum of $${limits.annualMax.toLocaleString()} reached.`);
  }

  return {
    claimAmount,
    fundPays: parseFloat(fundPays.toFixed(2)),
    memberPays: parseFloat(memberPays.toFixed(2)),
    deductibleApplied: deductible,
    coinsuranceRate: limits.coinsurance,
    withinLimits: fundPays > 0,
    notes
  };
};
...
// Uses dashboard version which computes investmentIncome — required for YearProjection type
// ─────────────────────────────────────────────
export const generateProjections = (assumptions: FinancialAssumptions): YearProjection[] => {
  ...
  for (let year = 1; year <= 5; year++) {
    // Determine average premium based on a weighted mix (simplified)
    // Assuming a mix that results in the input avgPremium (default $85)
    ...
    const combinedRatio = annualPremiumRevenue > 0
      ? ((expectedClaims + adminExpenses) / annualPremiumRevenue)
      : 0;
    ...
    members = members * (1 + assumptions.annualGrowthRate);
  }
...
// From dashboard engine — required by Reserves.tsx
// ─────────────────────────────────────────────

// Box-Muller transform for standard normal distribution (private helper)
...
// Simplified Monte Carlo for the frontend
export const runMonteCarlo = (
  ...
  for (let i = 0; i < simulations; i++) {
    // Simple Gaussian approximation for total claims
    // Mean = n * p * avg
    // StdDev approx = sqrt(n * p * (1-p)) * avg * volatility_factor

    // We will simulate the count of claims using a binomial approx
    // And simulate the severity using lognormal logic simplified

    // Simulate count of claims
    const claimCount = Math.round(members * frequency); // Simplified deterministic count for speed
    // Apply variance to claim count
    const varianceCount = Math.sqrt(claimCount);
    const actualCount = Math.max(0, claimCount + (generateGaussian() * varianceCount));

    // Simulate average claim size variance (Log-normal distribution characteristic)
    // We assume the average claim size fluctuates per year based on severity risk
    const severityFactor = Math.exp(generateGaussian() * 0.5); // 0.5 sigma
    const actualAvgClaim = avgClaim * severityFactor;

    const totalClaims = actualCount * actualAvgClaim;
