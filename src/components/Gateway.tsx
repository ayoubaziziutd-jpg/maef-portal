...
const Gateway: React.FC<GatewayProps> = ({ onSelectMember, onSelectEmployee }) => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden flex flex-col justify-end pb-16 px-6 bg-[#0a2e1a]">
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: `url('/logo.png')` }}
        aria-hidden
      />
      <div className="absolute inset-0 bg-[#0a2e1a]/40" aria-hidden />
      <div className="relative z-10 grid grid-cols-2 gap-10 w-full max-w-3xl mx-auto">
        <button
          onClick={() => setShowMemberPopup(true)}
          className="bg-black/40 backdrop-blur-sm border-2 border-amber-400/70 rounded-2xl py-10 px-6 hover:bg-black/60 transition-all hover:scale-[1.03] flex flex-col items-center justify-center gap-3"
        >
        ...
        <button
          ...
          className="bg-black/40 backdrop-blur-sm border-2 border-white/40 rounded-2xl py-10 px-6 hover:bg-black/60 transition-all hover:scale-[1.03] flex flex-col items-center justify-center gap-3"
        >
          ...
          <div className="bg-emerald-950 rounded-3xl p-10 w-full max-w-md mx-6 relative shadow-2xl border border-white/10">
            ...
            <div className="flex flex-col gap-4">
              <button
                ...
                className="flex items-center gap-5 bg-white/10 border border-amber-400/40 rounded-2xl p-6 hover:bg-white/20 transition-all"
              >
              ...
              <button
                onClick={onSelectMember}
                className="flex items-center gap-5 bg-white/10 border border-white/20 rounded-2xl p-6 hover:bg-white/20 transition-all"
              >
                <div className="w-14 h-14 rounded-full bg-emerald-400/20 flex items-center justify-center flex-shrink-0">
                  <Compass className="text-emerald-300" size={28} />
                </div>
                <div className="text-left">
